...
class GlobalConfigDatabaseMagicFolderTests(SyncTestCase):
    ...
    def test_create_folder_trailing_dot(self):
        """
        We can create folders that differ only in having a trailing dot in the name.

        Windows will strip a trailing dot from filenames, so test that we don't
        get state-directory colisions with names that differ only in a trailing
        dot.
        """
        config = create_global_configuration(self.temp, u"tcp:1234", self.node_dir, u"tcp:localhost:1234")
        alice = create_local_author(u"alice")
    ...
    def test_folder_state_already_exists(self):
        ...
        alice = create_local_author(u"alice")
        magic = self.temp.child("magic")
        state = config._get_default_state_path(name)
        magic.makedirs()
    ...
    def test_folder_get_path(self):
        ...
        self.assertThat(
            ...
            Equals(config._get_default_state_path(name).child(u"stash")),
        )
