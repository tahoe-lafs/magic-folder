...
from ..magic_folder import RemoteSnapshotCreator
...
from ..util.encoding import dump_yaml, load_yaml
...
class NodeDirectory(Fixture):
    ...
    @property
    def tahoe_cfg(self):
        return self.path.child("tahoe.cfg")

    @property
    def node_url(self):
        return self.path.child("node.url")

    @property
    def magic_folder_url(self):
        return self.path.child("magic-folder.url")

    @property
    def private(self):
        return self.path.child("private")

    @property
    def api_auth_token(self):
        return self.private.child("api_auth_token")

    @property
    def magic_folder_yaml(self):
        return self.private.child("magic_folders.yaml")
    ...
    def create_magic_folder(
        ...
        magic_folder_config.setdefault("magic-folders", {},)[folder_name] = {
...
class RemoteSnapshotCreatorFixture(Fixture):
    ...
    def _setUp(self):
        ...
        collective_dircap = "URI:DIR2-RO:mjrgeytcmjrgeytcmjrgeytcmi:mjrgeytcmjrgeytcmjrgeytcmjrgeytcmjrgeytcmjrgeytcmjra"
        ...
        self.config = MagicFolderConfig.initialize(
            ...
            self.author,
            self.stash_path,
            "URI:DIR2-RO:mjrgeytcmjrgeytcmjrgeytcmi:mjrgeytcmjrgeytcmjrgeytcmjrgeytcmjrgeytcmjrgeytcmjra",
            self.upload_dircap,
            self.magic_path,
            self.poll_interval,
            self.scan_interval,
...
class MagicFolderNode(object):
    ...
    def create(
            ...
            if folders:
                for name, config in folders.items():
                    global_config.create_magic_folder(
                        name,
                        config["magic-path"],
                        create_local_author(config["author-name"]),
                        "URI:DIR2{}:{}:{}".format(
                            "" if config["admin"] else "-RO",
                            b2a("\0" * 16),
                            b2a("\1" * 32),
                        ),
                        "URI:DIR2:{}:{}".format(b2a("\2" * 16), b2a("\3" * 32)),
        ...
        if folders and tahoe_root:
            ...
            for name, config in folders.items():
                ...
                if not config["admin"]:
                    ...
                    folder_config.collective_dircap = to_readonly_capability(
        ...
        http_client = create_testing_http_client(
            reactor, global_config, global_service, lambda: auth_token, status_service
        )
